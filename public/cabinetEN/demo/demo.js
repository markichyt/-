(function () {
  ...
  var MSG_ACTION = 'Demo mode: this action runs on the server and is switched off here.';
  var MSG_MODAL = 'The live cabinet fills this window from the server, so in a static demo it is empty.';
  ...
  onReady(function () {
    ...
    document.addEventListener('submit', function (e) {
      ...
      if (action.indexOf('#demo-action:') === 0 || method === 'post' || isBlocked(action)) {
        ...
        toast('Demo mode: the form is not sent to the server.');
      }
    }, true);
    ...
    // Modals that the live cabinet fills from the server are empty in the snapshot:
    // do not show a black screen — clear the state and explain instead.
