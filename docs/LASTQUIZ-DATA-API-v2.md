...
Слайд `fullProfile` идёт **перед выбором тарифа**. Табов больше нет — **все поля видны одновременно**. **Все поля опциональны** — юзер может пропустить слайд, но с настойчивым предупреждением.

**Сверху слайда — индикатор «Profile strength»** (0–100%), который пересчитывается в real-time:

| Поле | Вклад в strength |
|---|---|
| About yourself | до 50% (`min(chars/3000, 1) × 50`) |
| CV загружен | 30% |
| Company logo загружен | 10% |
| Referral code | 10% |

**Поведение Continue:**
- Если у юзера есть About ≥ 3000 chars **или** CV — переход дальше без вопросов.
- Если у юзера **нет ни того, ни другого** — открывается оранжево-красная модалка с предупреждением: «Ваш профайл будет слишком слабым, AI не сможет его сгенерировать качественно». Две кнопки: «← Go back and fill it in» (рекомендуемая) и «Skip anyway» (тихая, undeline-text). Юзер может пропустить, но видит явный риск.

> 💡 В CTA-блоке между About и CV-загрузкой объясняется, что AI-профайл генерируется напрямую из CV на английском — CV даёт +30% к strength и сильно повышает качество профайла.

| Поле | Тип | Required | Значения |
|---|---|---|---|
| `profile_method` | string | ✅ авто | `about_and_cv` \| `about` \| `cv_only` \| `skipped` |
| `about` | string | — | bio юриста (3000+ chars даёт максимум баллов в strength) |
...
⚠️ `profile_method` выставляется фронтом автоматически — это сигнал бэку, на основе чего AI сможет генерировать профайл:
- `about_and_cv` — есть и bio, и CV (лучший случай)
- `about` — только bio (3000+ chars)
- `cv_only` — только CV
- `skipped` — юзер ничего не заполнил, нужно подтолкнуть его пост-фактум (email-onboarding)
